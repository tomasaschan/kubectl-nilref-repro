...
import (
	...
	"github.com/tomasaschan/kubectl-nilref-repro/internal/controller"
...
var _ = BeforeSuite(func() {
	...
	err = (&controller.ReproReconciler{
		Client:       k8sClient,
		ChannelsPath: filepath.Join("..", "..", "channels"),
	}).SetupWithManager(manager)
	Expect(err).ToNot(HaveOccurred(), "failed to setup controller")

	go func() {
		defer GinkgoRecover()
		Expect(manager.Start(ctx)).To(WithTransform(func(err error) error {
